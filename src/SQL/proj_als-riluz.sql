...
select pr.*
from PCORNET_CDM.CDM.deid_prescribing pr
...
from PCORNET_CDM.CDM.deid_death dth 
join als_incld_demo als 
...
join PCORNET_CDM.CDM.deid_encounter b
on a.patid = b.patid
where b.admit_date::date <= '2025-08-01'
group by a.patid
;
...
select demo.*,
       ...
       o.outcome_status,
       o.outcome_date,
       case when datediff('day',demo.dx_date1,o.outcome_date)/365.25 <= 3 and o.outcome_status = 1 then 1 else 0 end as mort_1yr,
       case when datediff('day',demo.dx_date1,o.outcome_date)/365.25 <= 3 and o.outcome_status = 1 then 1 else 0 end as mort_3yr
from  als_incld_demo demo
...
select distinct code_grp from cci_ref;
...
with dx_all as (
    ...
    from als_incld_demo a
    join PCORNET_CDM.CDM.deid_diagnosis b
...
with fs_stk as (
    ...
    from als_incld_demo a 
    join PCORNET_CDM.CDM.deid_obs_clin b 
    on a.patid = b.patid
...
with sos_dx as (
    ...
    from als_incld_demo a
    join PCORNET_CDM.CDM.deid_procedures b
...
/* add new columns */
create or replace table als_riluzole_death_augmt as 
select als1.*,
       bmi.* exclude patid,
       sos.* exclude patid
from als_riluzole_death_final als1
join als_elig_bmi bmi 
on als1.patid = bmi.patid
join als_bl_sos sos 
on als1.patid = sos.patid
;

select * from als_riluzole_death_augmt limit 5;