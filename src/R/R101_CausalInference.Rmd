...
allvar<-c("AGE_AT_DX_DATE1", "RILUZOLE_DUR","SEX", "RACE", "RILUZOLE_IND","BMI","MORT_1YR", "time")

# create vector of categorical variable names
catvar<-c("SEX", "RACE", "RILUZOLE_IND","MORT_1YR")
...
```{r}
allvar_excld_strata<-allvar[!allvar %in% c("RILUZOLE_IND","RILUZOLE_DUR")]
catvar_excld_strata<-catvar[!catvar %in% c("RILUZOLE_IND","RILUZOLE_DUR")]
tab2<-CreateTableOne(vars=allvar_excld_strata,data=aset,factorVars = catvar_excld_strata,strata="RILUZOLE_IND")
tab2
```
...
tab3<-CreateTableOne(vars=allvar_excld_strata,data=aset2,factorVars = catvar_excld_strata,strata="RILUZOLE_IND")
tab3
```
...
```{r}
fit_cox<-coxph(Surv(time,status) ~ RILUZOLE_IND + AGE_AT_DX_DATE1 + CCS_101 + CCS_163 + CCS_211 + CCS_252 + CCS_53 + CCS_95,
               data=aset2)
summary(fit_cox)
```
...
```{r}
mod_match<-matchit(RILUZOLE_IND ~ CCS_101 + CCS_163 + CCS_211 + CCS_252 + CCS_53 + CCS_95, data = aset2,
...
fit_cox2<-coxph(Surv(time,status) ~ RILUZOLE_IND + AGE_AT_DX_DATE1,
                data=aset2_match)
summary(fit_cox2)
```
...
```{r}
mod_weight<-weightit(RILUZOLE_IND ~ CCS_101 + CCS_163 + CCS_211 + CCS_252 + CCS_53 + CCS_95, data = aset2)

fit_cox3<-coxph(Surv(time,status) ~ RILUZOLE_IND + AGE_AT_DX_DATE1,
