...
```{r}
new_var<-c(colnames(add_cci),colnames(add_fs))
...
```{r}
fit_cox<-coxph(
  Surv(time,status) ~ RILUZOLE_IND + SEX + AGE_AT_DX_DATE1 + BMI + CCI_TOT + RGS_WEAK  + SPEECH_ABN ,
  data=aset2
)
summary(fit_cox)
```
...
```{r}
mod_match<-matchit(
  RILUZOLE_IND ~ SEX + CCI_TOT + RGS_WEAK  + SPEECH_ABN, 
  data = aset2,
  method = "nearest", 
  distance = "glm"
)
