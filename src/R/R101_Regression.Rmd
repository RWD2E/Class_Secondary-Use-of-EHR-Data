...
```{r}
ggplot(aset %>% filter(RILUZOLE_DUR > 0), aes(x=AGE_AT_DX_DATE1,y=RILUZOLE_DUR))+
  geom_point()+
  geom_smooth(method="loess",formula = 'y~x')
```
...
```{r}
fit2<-glm(RILUZOLE_DUR ~ AGE_AT_DX_DATE1, 
...
```{r}
ggplot(aset2, aes(x=AGE_AT_DX_DATE1,y=MORT_1YR))+
  geom_point()+
  geom_smooth(method="loess",formula = 'y~x')
```
...
```{r}
fit3<-glm(MORT_1YR ~ AGE_AT_DX_DATE1, data=aset2,family='binomial') 
summary(fit3)
```
